      ...
      >
        <button
          ...
          className={`w-full flex items-center space-x-3 px-8 py-3 rounded-lg hover:bg-gray-100 transition-colors duration-200 ${
            isActive ? 'text-black bg-gray-100' : 'text-[#B1B1B1]'
          }`}
        >
          <span className={isActive ? 'text-black' : 'text-[#B1B1B1]'}>
            <Icon size={24} />
          </span>
      ...
      `}>
        ...
        <div className="p-6 pl-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-2xl font-bold text-[#343C6A]">Soar Task</h2>
            </div>
            {/* Close button for mobile */}
            <button 
              onClick={onClose}
              className="md:hidden w-10 h-10 rounded-full bg-[#F5F7FA] flex items-center justify-center hover:bg-gray-100 transition-colors"
            >
        ...
        <nav className="flex-1 py-4">
          <div className='relative'>
            ...
            <div className={`absolute left-0 h-full w-[8px] h-[56px] bg-black transition-all duration-300 ease-in-out rounded-r-lg duration-300`}
              style={{top: `${(hoveredIndex ?? 0) * 56 - 5}px`}}
              />
          </div>
        </nav>
      </aside>
